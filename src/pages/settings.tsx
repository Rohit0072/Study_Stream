...
import { ArrowLeft, Save, Key, Globe, Volume2, Moon, Sun, Info, Play } from "lucide-react"
import { useNavigate } from "react-router-dom"
import { useApp } from "../context/app-context"

export default function SettingsPage() {
  const navigate = useNavigate()
  const { geminiApiKey, setGeminiApiKey, userProfile, updateUserProfile } = useApp()
  const [apiKey, setApiKey] = useState(geminiApiKey || "")
  const [isDarkMode, setIsDarkMode] = useState(true)
  const [savedMessage, setSavedMessage] = useState("")
  const [showApiKeyInfo, setShowApiKeyInfo] = useState(false)
  const [autoPlayNext, setAutoPlayNext] = useState(userProfile.autoPlayNext || false)

  useEffect(() => {
    setApiKey(geminiApiKey || "")
    setAutoPlayNext(userProfile.autoPlayNext || false)
  }, [geminiApiKey, userProfile.autoPlayNext])
  ...
  const handleToggleAutoPlay = () => {
    ...
    updateUserProfile({ autoPlayNext: newValue })
        ...
        <div className="max-w-2xl mx-auto">
          ...
          >
            <div className="flex items-center gap-3 mb-4">
              ...
              <h2 className="text-xl font-semibold text-white">Video Playback</h2>
            </div>
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-gray-400 text-sm">Automatically play the next video when current video ends</p>
                </div>
                <button
                  onClick={handleToggleAutoPlay}
                  className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
                    autoPlayNext ? "bg-purple-600" : "bg-gray-600"
                  }`}
                >
                  <span
