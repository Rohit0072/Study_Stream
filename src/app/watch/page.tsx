...
function WatchContent() {
    ...
    const handleBookmark = useCallback((time: number) => {
        ...
        if (!currentVideo) {
            console.error('[WatchPage] CurrentVideo is missing');
            return;
        }
        ...
        const newBookmark = {
            ...
            time: time,
            label: `Bookmark at ${Math.floor(time)}s`,
            createdAt: Date.now()
        };
    ...
    }, [currentVideo?.id, currentVideo?.progress, currentVideo?.duration]);
